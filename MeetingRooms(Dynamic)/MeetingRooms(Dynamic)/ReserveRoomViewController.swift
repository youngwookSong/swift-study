...
class ReserveRoomViewController: UITableViewController {
    ...
    @IBAction func done(_ sender: Any) {
        guard let reservation = newReservatio() else{
            self.dismiss(animated: true, completion: nil)
            return
        }
        ...
        case let tabBarC as UITabBarController:
            if let navigationC = tabBarC.selectedViewController as? UINavigationController, let reservationListVC = navigationC.topViewController as? ReservationListViewController { reservationListVC.addNewItem(reservation: reservation)
            }
        case let navigationC as UINavigationController:
            if let reservationListVC = navigationC.topViewController as?
    ...
    func newReservatio() -> Reservation? {
        ...
        reservation.hostNames = host
        reservation.date = DatePicker.date as NSDate //이케 써야되네
